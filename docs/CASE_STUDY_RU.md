...
Автор: [t.me/ei_ai_channel](https://t.me/ei_ai_channel)
...
6. Добавили hold-to-talk через `Right Option` (по умолчанию):
- зажал `Right Option` -> идет запись;
- отпустил `Right Option` -> стоп и вставка;
- fallback на keycode 61 для macOS, где правый `Option` приходит нестабильно.
...
- Сейчас кейс реализован для macOS.
- Для Windows нужен отдельный слой под хоткеи, доступность и вставку текста.
